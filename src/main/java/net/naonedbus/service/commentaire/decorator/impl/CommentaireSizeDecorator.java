...
/**
 * Raccourcit le message d'un commentaire s'il est trop long.
 * @author Benoît
 * @version $Revision$ $Date$
 */
public class CommentaireSizeDecorator
    implements CommentaireDecorator
{
    ...
    {
        if (commentaire.getMessage().length() > NaonedbusConstants.COMMENTAIRE_TAILLE)
        {
            commentaire.setMessage(commentaire
                    .getMessage()
                    .substring(0,
                               NaonedbusConstants.COMMENTAIRE_TAILLE));
        }
    }
}
